...
use crate::dpx_pdfobj::{
    pdf_link_obj, pdf_new_array, pdf_new_dict, pdf_new_null,
        ...
        for i in 0..num_leaves {
            ...
            match pdf_obj_typeof((*cur).value) {
                PdfObjType::ARRAY | PdfObjType::DICT | PdfObjType::STREAM | PdfObjType::STRING => {
                    (*names).as_array_mut().push(pdf_ref_obj((*cur).value));
                }
                ...
                _ => {
                    (*names).as_array_mut().push(pdf_link_obj((*cur).value));
                }
            }
