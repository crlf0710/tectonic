...
pub unsafe fn cff_open<'a>(
    ...
    (*cff).index = n;
    (*cff).handle = Some(handle);
    (*cff).offset = offset as l_offset;
    (*cff).filter = 0i32;
    (*cff).flag = 0i32;
    (*cff).name = ptr::null_mut();
    (*cff).topdict = ptr::null_mut();
    (*cff).gsubr = ptr::null_mut();
    ...
    let handle = &mut (*cff).handle.as_mut().unwrap();
    handle.seek(SeekFrom::Start((*cff).offset as u64)).unwrap();
    (*cff).header.major = tt_get_unsigned_byte(handle);
    (*cff).header.minor = tt_get_unsigned_byte(handle);
    (*cff).header.hdr_size = tt_get_unsigned_byte(handle);
    (*cff).header.offsize = tt_get_unsigned_byte(handle);
    ...
    handle.seek(SeekFrom::Start((*cff).offset as u64 + (*cff).header.hdr_size as u64)).unwrap();
    /* Name INDEX */
    let idx = cff_get_index(&mut *cff);
    ...
    handle.seek(SeekFrom::Start((*cff).offset as u64 + offset as u64)).unwrap();
    ...
    handle.seek(SeekFrom::Start((*cff).offset as u64 + (*cff).gsubr_offset as u64)).unwrap(); /* no trailing '\0' */
    return cff; /* Additional data in between header and
                 * Name INDEX ignored.
                 */
}

pub unsafe fn cff_close(mut cff: *mut cff_font) {
    if !cff.is_null() {
...
pub unsafe fn cff_get_name(cff: &cff_font) -> *mut i8 {
    let idx = cff.name;
...
pub unsafe fn cff_set_name(mut cff: *mut cff_font, mut name: *mut i8) -> i32 {
    if strlen(name) > 127 {
...
pub unsafe fn cff_put_header(cff: &cff_font, mut dest: &mut [u8]) -> usize {
...
pub unsafe fn cff_get_index_header(cff: &mut cff_font) -> *mut cff_index {
    let idx = new((1_u64).wrapping_mul(::std::mem::size_of::<cff_index>() as u64) as u32)
        as *mut cff_index;
    let handle = cff.handle.as_mut().unwrap();
    let count = tt_get_unsigned_pair(handle);
    (*idx).count = count;
    if count as i32 > 0i32 {
        (*idx).offsize = tt_get_unsigned_byte(handle);
        ...
        if count as i32 == 0xffffi32 {
            ...
            handle.seek(SeekFrom::Start(n + (*idx).offsize as u64)).unwrap();
        } else {
...
pub unsafe fn cff_get_index(cff: &mut cff_font) -> *mut cff_index {
    let idx = new((1_u64).wrapping_mul(::std::mem::size_of::<cff_index>() as u64) as u32)
        as *mut cff_index;
    let handle = cff.handle.as_mut().unwrap();
    let count = tt_get_unsigned_pair(handle);
...
pub unsafe fn cff_pack_index(mut idx: *mut cff_index, mut dest: &mut [u8]) -> usize {
    let destlen = dest.len();
...
pub unsafe fn cff_index_size(mut idx: *mut cff_index) -> usize {
...
pub unsafe fn cff_new_index(mut count: u16) -> *mut cff_index {
    let idx = new((1_u64).wrapping_mul(::std::mem::size_of::<cff_index>() as u64) as u32)
        as *mut cff_index;
    (*idx).count = count;
    (*idx).offsize = 0i32 as c_offsize;
...
pub unsafe fn cff_release_index(mut idx: *mut cff_index) {
    if !idx.is_null() {
        if !(*idx).data.is_null() {
...
pub unsafe fn cff_get_string(mut cff: *const cff_font, mut id: s_SID) -> *mut i8 {
...
pub unsafe fn cff_get_sid(mut cff: *mut cff_font, mut str: *const i8) -> i32 {
...
pub unsafe fn cff_get_seac_sid(mut cff: *mut cff_font, mut str: *const i8) -> i32 {
    if cff.is_null() || str.is_null() {
        return -1i32;
    }
    for i in 0..391 {
...
pub unsafe fn cff_update_string(cff: &mut cff_font) {
    /*if cff.is_null() {
...
pub unsafe fn cff_read_encoding(cff: &mut cff_font) -> i32 {
    if cff.topdict.is_null() {
    ...
    (*encoding).format = tt_get_unsigned_byte(handle);
    let mut length = 1;
    match (*encoding).format as i32 & !0x80i32 {
        0 => {
            (*encoding).num_entries = tt_get_unsigned_byte(handle);
            (*encoding).data.codes = new(((*encoding).num_entries as u32 as u64)
                .wrapping_mul(::std::mem::size_of::<u8>() as u64)
                as u32) as *mut u8;
            for i in 0..(*encoding).num_entries {
                *(*encoding).data.codes.offset(i as isize) = tt_get_unsigned_byte(handle);
            }
        ...
        1 => {
            ...
            for i in 0..(*encoding).num_entries {
                (*ranges.offset(i as isize)).first = tt_get_unsigned_byte(handle) as s_SID;
                (*ranges.offset(i as isize)).n_left = tt_get_unsigned_byte(handle);
            }
    ...
    if (*encoding).format as i32 & 0x80i32 != 0 {
        (*encoding).num_supps = tt_get_unsigned_byte(handle);
        ...
        for i in 0..(*encoding).num_supps {
            (*map.offset(i as isize)).code = tt_get_unsigned_byte(handle);
            (*map.offset(i as isize)).glyph = tt_get_unsigned_pair(handle);
            /* SID */
        }
...
pub unsafe fn cff_pack_encoding(cff: &cff_font, dest: &mut [u8]) -> usize {
    let mut len = 0_usize;
...
pub unsafe fn cff_encoding_lookup(cff: &cff_font, mut code: u8) -> u16 {
...
pub unsafe fn cff_release_encoding(mut encoding: *mut cff_encoding) {
    if !encoding.is_null() {
        match (*encoding).format as i32 & !0x80i32 {
            0 => {
...
pub unsafe fn cff_read_charsets(cff: &mut cff_font) -> i32 {
    if cff.topdict.is_null() {
    ...
    (*charset).format = tt_get_unsigned_byte(handle);
    ...
    match (*charset).format as i32 {
        0 => {
            ...
            for i in 0..(*charset).num_entries {
                *(*charset).data.glyphs.offset(i as isize) = tt_get_unsigned_pair(handle);
            }
            count = 0i32 as u16
        }
        1 => {
...
pub unsafe fn cff_pack_charsets(cff: &cff_font, dest: &mut [u8]) -> usize {
    let destlen = dest.len();
    let mut len = 0;
...
pub unsafe fn cff_get_glyphname(cff: &cff_font, gid: u16) -> *mut i8 {
    let sid = cff_charsets_lookup_inverse(cff, gid);
    cff_get_string(cff, sid)
}

pub unsafe fn cff_glyph_lookup(cff: &cff_font, mut glyph: *const i8) -> u16 {
...
pub unsafe fn cff_charsets_lookup(cff: &cff_font, mut cid: u16) -> u16 {
    if cff.flag & (1i32 << 5i32 | 1i32 << 6i32 | 1i32 << 7i32) != 0 {
        panic!("Predefined CFF charsets not supported yet");
    } else {
        if cff.charsets.is_null() {
...
pub unsafe fn cff_charsets_lookup_gid(
...
pub unsafe fn cff_charsets_lookup_inverse(cff: &cff_font, gid: u16) -> u16 {
    if cff.flag & (1i32 << 5i32 | 1i32 << 6i32 | 1i32 << 7i32) != 0 {
        panic!("Predefined CFF charsets not supported yet");
    } else {
        if cff.charsets.is_null() {
...
pub unsafe fn cff_charsets_lookup_cid(
    mut charset: *mut cff_charsets,
    mut gid: u16,
) -> u16 {
...
pub unsafe fn cff_release_charsets(mut charset: *mut cff_charsets) {
    if !charset.is_null() {
        match (*charset).format as i32 {
            0 => {
...
pub unsafe fn cff_read_fdselect(cff: &mut cff_font) -> i32 {
    if cff.topdict.is_null() {
        panic!("Top DICT not available");
    }
    ...
    let fdsel = new((1_u64).wrapping_mul(::std::mem::size_of::<cff_fdselect>() as u64) as u32)
        as *mut cff_fdselect;
    cff.fdselect = fdsel;
    (*fdsel).format = tt_get_unsigned_byte(handle);
    let mut length = 1;
    match (*fdsel).format as i32 {
        0 => {
            ...
            for i in 0..(*fdsel).num_entries as i32 {
                *(*fdsel).data.fds.offset(i as isize) = tt_get_unsigned_byte(handle);
            }
            length += (*fdsel).num_entries as i32
        }
        3 => {
            (*fdsel).num_entries = tt_get_unsigned_pair(handle);
            ...
            for i in 0..(*fdsel).num_entries as i32 {
                (*ranges.offset(i as isize)).first = tt_get_unsigned_pair(handle);
                (*ranges.offset(i as isize)).fd = tt_get_unsigned_byte(handle);
            }
            if (*ranges.offset(0)).first as i32 != 0i32 {
                panic!("Range not starting with 0.");
            }
            if cff.num_glyphs as i32 != tt_get_unsigned_pair(handle) as i32 {
...
pub unsafe fn cff_pack_fdselect(cff: &cff_font, dest: &mut [u8]) -> usize {
    let mut len = 0;
    if cff.fdselect.is_null() {
        return 0;
    }
...
pub unsafe fn cff_release_fdselect(mut fdselect: *mut cff_fdselect) {
    if !fdselect.is_null() {
...
pub unsafe fn cff_fdselect_lookup(cff: &cff_font, mut gid: u16) -> u8 {
    if cff.fdselect.is_null() {
...
pub unsafe fn cff_read_subrs(cff: &mut cff_font) -> i32 {
    ...
    if cff.gsubr.is_null() {
        cff.handle.as_mut().unwrap().seek(SeekFrom::Start(cff.offset as u64 + cff.gsubr_offset as u64)).unwrap();
        ...
        for i in 0..cff.num_fds as i32 {
                ...
                ) == 0
            {
                ...
                *fresh47 = ptr::null_mut()
                ...
                cff.handle.as_mut().unwrap().seek(SeekFrom::Start(cff.offset as u64 + offset as u64)).unwrap();
...
pub unsafe fn cff_read_fdarray(cff: &mut cff_font) -> i32 {
    if cff.topdict.is_null() {
...
pub unsafe fn cff_read_private(cff: &mut cff_font) -> i32 {
    let mut len: i32 = 0i32;
    let mut size: i32 = 0;
    if cff.flag & 1i32 << 0i32 != 0 {
        if cff.fdarray.is_null() {
