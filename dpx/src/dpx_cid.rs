...
use crate::dpx_pdfobj::{
    pdf_copy_name, pdf_file, pdf_get_version, pdf_link_obj,
...
pub unsafe fn CIDFont_get_resource(mut font: *mut CIDFont) -> *mut pdf_obj {
    assert!(!font.is_null());
    if (*font).indirect.is_null() {
...
pub unsafe fn CIDFont_cache_close() {
    if !__cache.is_null() {
