...
pub(crate) unsafe fn new_null_box() -> i32 {
    let p = get_node(BOX_NODE_SIZE) as usize;
...
pub(crate) unsafe fn new_rule() -> i32 {
    let p = get_node(RULE_NODE_SIZE) as usize;
...
pub(crate) unsafe fn new_ligature(mut f: internal_font_number, mut c: u16, mut q: i32) -> i32 {
    let p = get_node(SMALL_NODE_SIZE) as usize;
...
pub(crate) unsafe fn new_math(mut w: scaled_t, mut s: small_number) -> i32 {
    let p = get_node(MEDIUM_NODE_SIZE) as usize;
    *NODE_type(p) = MATH_NODE;
    MEM[p].b16.s0 = s as u16;
    MEM[p + 1].b32.s1 = w;
    p as i32
}
pub(crate) unsafe fn new_spec(p: usize) -> i32 {
    let q = get_node(GLUE_SPEC_SIZE) as usize;
    MEM[q] = MEM[p];
...
pub(crate) unsafe fn new_glue(mut q: i32) -> i32 {
    let p = get_node(MEDIUM_NODE_SIZE) as usize;
    *NODE_type(p) = GLUE_NODE;
    *GLUE_SPEC_shrink_order(p as usize) = NORMAL;
    MEM[p + 1].b32.s1 = TEX_NULL;
    MEM[p + 1].b32.s0 = q;
...
pub(crate) unsafe fn delete_glue_ref(mut p: i32) {
    if MEM[p as usize].b32.s1.is_texnull() {
        free_node(p as usize, GLUE_SPEC_SIZE);
    } else {
...
pub(crate) unsafe fn push_nest() {
    ...
    NEST_PTR += 1;
    cur_list.head = get_avail() as i32;
...
pub(crate) unsafe fn scan_something_internal(mut level: small_number, mut negative: bool) {
    ...
    if negative {
        if cur_val_level >= GLUE_VAL {
            cur_val = new_spec(cur_val as usize);
...
pub(crate) unsafe fn scan_general_text() {
    ...
    let q = MEM[def_ref as usize].b32.s1;
...
pub(crate) unsafe fn new_native_character(
    ...
    if !(FONT_MAPPING[f]).is_null() {
        ...
        p = get_node(NATIVE_NODE_SIZE + 1) as i32;
...
pub(crate) unsafe fn load_native_font(
    ...
    *font_used.offset(FONT_PTR as isize) = false;
    ...
    free_node(p as usize, *NATIVE_NODE_size(p as usize) as i32);
        ...
        match current_block {
            ...
            _ => {
                ...
                if MEM[LR_ptr as usize].b32.s0 != BEFORE as i32 {
                    ...
                    loop {
                        temp_ptr = q;
                        q = new_math(0, MEM[LR_ptr as usize].b32.s0 as small_number);
...
pub(crate) unsafe fn append_to_vlist(mut b: i32) {
    ...
    if cur_list.aux.b32.s1 > IGNORE_DEPTH {
        ...
        if d < *DIMENPAR(DimenPar::line_skip_limit) {
            p = new_param_glue(GluePar::line_skip as small_number)
        } else {
            p = new_skip_param(GluePar::baseline_skip as small_number);
...
pub(crate) unsafe fn new_noad() -> i32 {
    let p = get_node(NOAD_SIZE) as usize;
...
pub(crate) unsafe fn init_align() {
    ...
    align_state = -1000000;
    loop {
        MEM[cur_align as usize].b32.s1 = new_param_glue(GluePar::tab_skip as small_number);
        /*:808 */
...
pub(crate) unsafe fn init_row() {
    ...
    MEM[cur_list.tail as usize].b32.s1 =
        new_glue(MEM[(MEM[ALIGN_HEAD].b32.s1 + 1) as usize].b32.s0);
    cur_list.tail = *LLIST_link(cur_list.tail as usize);
    MEM[cur_list.tail as usize].b16.s0 = GluePar::tab_skip as u16 + 1;
...
pub(crate) unsafe fn fin_col() -> bool {
    ...
    if MEM[(cur_align + 5) as usize].b32.s0 != SPAN_CODE {
        ...
        *NODE_type(u as usize) = UNSET_NODE;
        ...
        cur_list.tail = *LLIST_link(cur_list.tail as usize);
        MEM[cur_list.tail as usize].b16.s0 = 12;
...
pub(crate) unsafe fn fin_align() {
    ...
    if cur_list.mode == MMODE {
        ...
        cur_list.tail = *LLIST_link(cur_list.tail as usize);
        MEM[cur_list.tail as usize].b32.s1 =
            new_param_glue(GluePar::above_display_skip as small_number);
        cur_list.tail = *LLIST_link(cur_list.tail as usize);
        MEM[cur_list.tail as usize].b32.s1 = p;
        if !p.is_texnull() {
            cur_list.tail = q;
        ...
        cur_list.tail = *LLIST_link(cur_list.tail as usize);
        MEM[cur_list.tail as usize].b32.s1 =
            new_param_glue(GluePar::below_display_skip as small_number);
        cur_list.tail = *LLIST_link(cur_list.tail as usize);
            ...
            11492179201936201469 => {
                ...
                if *NODE_type(p as usize) == KERN_NODE {
                    ...
                    if MEM[q as usize].b16.s0 != NORMAL && MEM[(q + 3) as usize].b32.s1 != 0 {
                        ...
                        error();
                        r = new_spec(q as usize);
                        *GLUE_SPEC_shrink_order(r as usize) = NORMAL;
                        delete_glue_ref(q);
...
pub(crate) unsafe fn vsplit(mut n: i32, mut h: scaled_t) -> i32 {
    ...
    if !cur_mark[SPLIT_FIRST_MARK_CODE as usize].is_texnull() {
        delete_token_ref(cur_mark[SPLIT_FIRST_MARK_CODE as usize]);
        cur_mark[SPLIT_FIRST_MARK_CODE as usize] = TEX_NULL;
        delete_token_ref(cur_mark[SPLIT_BOT_MARK_CODE as usize]);
    ...
    if !q.is_texnull() {
        q = vpackage(q, 0, ADDITIONAL as small_number, MAX_HALFWORD)
    }
    if cur_val < 256 {
    ...
    vpackage(p, h, EXACTLY as i16, *DIMENPAR(DimenPar::split_max_depth))
}
...
pub(crate) unsafe fn app_space() {
    ...
    if cur_list.aux.b32.s0 >= 2000 && *GLUEPAR(GluePar::xspace_skip) != 0i32 {
        ...
        main_p = new_spec(main_p as usize);
        ...
        MEM[(main_p + 2) as usize].b32.s1 =
            xn_over_d(MEM[(main_p + 2) as usize].b32.s1, cur_list.aux.b32.s0, 1000);
        MEM[(main_p + 3) as usize].b32.s1 =
            xn_over_d(MEM[(main_p + 3) as usize].b32.s1, 1000, cur_list.aux.b32.s0);
        q = new_glue(main_p);
...
pub(crate) unsafe fn its_all_over() -> bool {
    if privileged() {
        ...
        cur_list.tail = *LLIST_link(cur_list.tail as usize);
        MEM[cur_list.tail as usize].b32.s1 = new_penalty(NULL_FLAG);
        cur_list.tail = *LLIST_link(cur_list.tail as usize);
        build_page();
    }
    false
...
pub(crate) unsafe fn append_glue() {
    ...
    if s >= SKIP_CODE as i16 {
        MEM[cur_val as usize].b32.s1 -= 1;
        if s > SKIP_CODE as i16 {
            MEM[cur_list.tail as usize].b16.s0 = MU_GLUE;
        }
    };
}
            ...
            if cur_list.mode.abs() == VMODE {
                if !pre_adjust_tail.is_texnull() {
                    if PRE_ADJUST_HEAD as i32 != pre_adjust_tail {
                        MEM[cur_list.tail as usize].b32.s1 = MEM[PRE_ADJUST_HEAD].b32.s1;
                ...
                if !adjust_tail.is_texnull() {
                    if ADJUST_HEAD as i32 != adjust_tail {
                        MEM[cur_list.tail as usize].b32.s1 = MEM[ADJUST_HEAD].b32.s1;
                ...
                if cur_list.mode.abs() == HMODE {
                    ...
                    p = new_noad();
                    MEM[(p + 1) as usize].b32.s1 = SUB_BOX;
...
pub(crate) unsafe fn new_graf(mut indented: bool) {
    ...
    if cur_list.mode == VMODE || cur_list.head != cur_list.tail {
        MEM[cur_list.tail as usize].b32.s1 = new_param_glue(GluePar::par_skip as small_number);
        cur_list.tail = *LLIST_link(cur_list.tail as usize)
    }
    push_nest();
    cur_list.mode = HMODE;
...
pub(crate) unsafe fn append_italic_correction() {
    ...
    if cur_list.tail != cur_list.head {
        if is_char_node(cur_list.tail) {
            p = cur_list.tail
        } else if MEM[cur_list.tail as usize].b16.s1 == LIGATURE_NODE {
            p = cur_list.tail + 1;
        } else if *NODE_type(cur_list.tail as usize) == WHATSIT_NODE {
            if MEM[cur_list.tail as usize].b16.s0 == NATIVE_WORD_NODE
                || MEM[cur_list.tail as usize].b16.s0 == NATIVE_WORD_NODE_AT
            {
                MEM[cur_list.tail as usize].b32.s1 = new_kern(real_get_native_italic_correction(
                    &mut MEM[cur_list.tail as usize] as *mut memory_word as *mut libc::c_void,
                ));
                cur_list.tail = *LLIST_link(cur_list.tail as usize);
                *NODE_subtype(cur_list.tail as usize) = EXPLICIT
            } else if MEM[cur_list.tail as usize].b16.s0 == GLYPH_NODE {
                MEM[cur_list.tail as usize].b32.s1 =
        ...
        f = *CHAR_NODE_font(p as usize) as internal_font_number;
        MEM[cur_list.tail as usize].b32.s1 = new_kern(*FONT_CHARINFO_ITALCORR(
            f,
            FONT_CHARACTER_INFO(
                f,
                effective_char(true, f, *CHAR_NODE_character(p as usize)) as usize,
        ...
        cur_list.tail = *LLIST_link(cur_list.tail as usize);
        MEM[cur_list.tail as usize].b16.s0 = EXPLICIT
    };
}
...
pub(crate) unsafe fn make_accent() {
    ...
    let mut p = new_character(f, cur_val as UTF16_code);
    if !p.is_texnull() {
        ...
        if !q.is_texnull() {
            ...
            if FONT_AREA[f] as u32 == AAT_FONT_FLAG || FONT_AREA[f] as u32 == OTGR_FONT_FLAG {
                ...
                i = FONT_CHARACTER_INFO(
                    f,
                    effective_char(true, f, *CHAR_NODE_character(q as usize)) as usize,
                );
                w = *FONT_CHARINFO_WIDTH(f, i);
                h = *FONT_CHARINFO_HEIGHT(f, i);
            }
            if h != x {
                p = hpack(p, 0, ADDITIONAL as small_number);
            ...
            *NODE_subtype(r as usize) = ACC_KERN;
            ...
            MEM[cur_list.tail as usize].b16.s0 = ACC_KERN;
                ...
                _ => {
                    ...
                    match cur_list.mode.abs() + cur_cmd as i16 {
                            ...
                            if cur_chr > 0 {
                                if eTeX_enabled(
                                    *INTPAR(IntPar::texxet) > 0,
                                    ...
                                    MEM[cur_list.tail as usize].b32.s1 =
                                        new_math(0, cur_chr as small_number);
                                    cur_list.tail = *LLIST_link(cur_list.tail as usize)
                                }
                            } else {
                                init_align();
                        ...
                        262 => {
                            ...
                            cur_list.tail = *LLIST_link(cur_list.tail as usize);
                            MEM[cur_list.tail as usize].b16.s0 = COND_MATH_GLUE;
                            continue 'c_125208;
                        }
                        261 => {
                            append_choices();
                            ...
                            if !is_char_node(main_p) && *NODE_type(main_p as usize) == GLUE_NODE {
                                ...
                                if main_ppp == cur_list.tail {
                                    ...
                                    *(&mut MEM[(temp_ptr + 6) as usize] as *mut memory_word
                                    ...
                                    t = 0;
                                    let mut for_end_8 =
                                        MEM[(cur_list.tail + 4) as usize].b16.s1 as i32 - 1;
                                    if t <= for_end_8 {
                                        loop {
                                            *(&mut MEM[(temp_ptr + 6) as usize]
                                                as *mut memory_word
        ...
        match current_block {
            ...
            /*append_normal_space */
            {
                ...
                if *GLUEPAR(GluePar::space_skip) == 0 {
                    ...
                    temp_ptr = new_param_glue(GluePar::space_skip as small_number)
                }
