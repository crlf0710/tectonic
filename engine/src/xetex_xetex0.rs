...
pub(crate) unsafe fn new_skip_param(n: GluePar) -> usize {
    ...
    MEM[temp_ptr as usize].b32.s1 = None.tex_int();
    MEM[p].b16.s0 = n as u16 + 1;
    p
}
...
pub(crate) unsafe fn get_next() {
    ...
    'c_63502: loop {
        ...
        if cur_cmd <= Cmd::CarRet && cur_cmd >= Cmd::TabMark && align_state == 0 {
            ...
            if let Some(a) = cur_align.opt() {
                cur_cmd = Cmd::from(MEM[a + 5].b32.s0 as u16);
                MEM[a + 5].b32.s0 = cur_chr;
                if cur_cmd == Cmd::Omit {
                    begin_token_list(OMIT_TEMPLATE, Btl::VTemplate);
                } else {
                    begin_token_list(MEM[a + 2].b32.s1 as usize, Btl::VTemplate);
                }
                align_state = 1000000;
            } else {
                fatal_error(b"(interwoven alignment preambles are not allowed)");
...
pub(crate) unsafe fn find_sa_element(t: ValLevel, mut n: i32, mut w: bool) {
    cur_ptr = sa_root[t as usize].tex_int();
    ...
    cur_ptr = if i as i32 & 1i32 != 0 {
        MEM[(q + i as i32 / 2 + 1) as usize].b32.s1
    } else {
    ...
    unsafe fn not_found(t: ValLevel, n: i32) {
        ...
        sa_root[t as usize] = cur_ptr.opt();
        let q = cur_ptr;
        let i = (n / 0x40000) as i16;
        lab46(t, n, q, i)
    }
        ...
        if cur_cmd < Cmd::Call {
            ...
            match cur_cmd {
                Cmd::TopBotMark => {
                    ...
                    if cur_val == 0 {
                        cur_ptr = cur_mark[t as usize].tex_int();
                    } else {
...
pub(crate) unsafe fn show_info() {
    show_node_list(MEM[temp_ptr as usize].b32.s0.opt());
}
...
pub(crate) unsafe fn pop_alignment() {
    MEM[cur_head.unwrap()].b32.s1 = avail;
    avail = cur_head.tex_int();
    MEM[cur_pre_head.unwrap()].b32.s1 = avail;
    avail = cur_pre_head.tex_int();
...
pub(crate) unsafe fn fin_col() -> bool {
    ...
    let mut q = MEM[cur_align as usize].b32.s1.opt().confuse(b"endv") as i32;
...
pub(crate) unsafe fn app_space() {
    ...
    if cur_list.aux.b32.s0 >= 2000 && *GLUEPAR(GluePar::xspace_skip) != 0i32 {
        ...
        q = new_glue(main_p);
        MEM[main_p_].b32.s1 = None.tex_int()
    }
...
pub(crate) unsafe fn box_end(mut box_context: i32) {
    ...
    if box_context < BOX_FLAG {
        ...
        if cur_val < 256 {
            if a >= 4 {
                geq_define(BOX_BASE + cur_val as usize, Cmd::BoxRef, cur_box.opt());
            } else {
                eq_define(BOX_BASE + cur_val as usize, Cmd::BoxRef, cur_box.opt());
            }
        } else {
            find_sa_element(ValLevel::Ident, cur_val, true);
            if a >= 4 {
                gsa_def(cur_ptr as usize, cur_box.opt());
            } else {
                sa_def(cur_ptr as usize, cur_box.opt());
            }
        }
        ...
        BOX_CODE => {
            ...
            if cur_val < 256 {
                *BOX_REG(cur_val as usize) = None.tex_int()
                ...
                find_sa_element(ValLevel::Ident, cur_val, false);
                if !cur_ptr.is_texnull() {
                    MEM[(cur_ptr + 1) as usize].b32.s1 = None.tex_int();
                    MEM[(cur_ptr + 1) as usize].b32.s0 += 1;
                    delete_sa_ref(cur_ptr as usize);
                }
            }
...
pub(crate) unsafe fn just_reverse(mut p: i32) {
    ...
    if MEM[TEMP_HEAD].b32.s1.is_texnull() {
        just_copy(MEM[p as usize].b32.s1.opt(), TEMP_HEAD, None.tex_int());
