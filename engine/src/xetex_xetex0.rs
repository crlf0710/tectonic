                ...
                if PAGE_HEAD != page_tail as usize {
                    ...
                    if page_contents as i32 > EMPTY {
                        ...
                        while r != PAGE_INS_HEAD {
                            ...
                            print_scaled(t);
                            if MEM[r].b16.s1 as i32 == SPLIT_UP.u16() as i32 {
                                let mut q = PAGE_HEAD;
                                t = 0;
                                loop {
...
pub(crate) unsafe fn print_cmd_chr(mut cmd: Cmd, mut chr_code: i32) {
    ...
    match cmd {
        ...
        MATH_COMP => match ND::from(chr_code as u16) {
            ORD_NOAD => print_esc_cstr(b"mathord"),
            OP_NOAD => print_esc_cstr(b"mathop"),
            BIN_NOAD => print_esc_cstr(b"mathbin"),
            REL_NOAD => print_esc_cstr(b"mathrel"),
        ...
        LEFT_RIGHT => {
            if chr_code as u16 == LEFT_NOAD.u16() {
                print_esc_cstr(b"left");
        ...
        CALL | LONG_CALL | OUTER_CALL | LONG_OUTER_CALL => {
            let mut n = cmd as u8 - CALL as u8;
...
pub(crate) unsafe fn macro_call() {
    ...
    if MEM[r as usize].b32.s0 != END_MATCH_TOKEN {
        ...
        if long_state as u16 >= OUTER_CALL as u16 {
            ...
            'c_67378: loop {
                ...
                if cur_tok == MEM[r as usize].b32.s0 {
                    ...
                    if cur_tok == par_token {
                        if long_state as u16 != LONG_CALL as u16 {
                            /*414:*/
                            if long_state as u16 == CALL as u16 {
                                runaway(); /*411:*/
                                if file_line_error_style_p != 0 {
                                    print_file_line(); /*413:*/
                                } else {
            ...
            match cur_cmd {
                ...
                EXPAND_AFTER => {
                    ...
                    if cur_chr == 0 {
                        ...
                        get_token();
                        if cur_cmd == IF_TEST && cur_chr != IF_CASE_CODE as i32 {
                ...
                CS_NAME => {
                    ...
                    flush_list(Some(r));
                    if Cmd::from(EQTB[cur_cs as usize].cmd) == UNDEFINED_CS {
...
pub(crate) unsafe fn set_math_char(mut c: i32) {
    ...
    if c as u32 & 0x1fffff_u32 == ACTIVE_MATH_CHAR as u32 {
        ...
        if c as u32 >> 21i32 & 0x7_u32 == 7 {
            ...
            MEM[p].b16.s1 = ORD_NOAD.u16()
        } else {
            MEM[p].b16.s1 = (ORD_NOAD.u16() as u32).wrapping_add(c as u32 >> 21 & 0x7_u32) as u16
        }
...
pub(crate) unsafe fn conditional() {
    ...
    match this_if {
        IF_CHAR_CODE | IF_CAT_CODE => {
            ...
            let (m, n) = if cur_cmd > ACTIVE_CHAR || cur_chr > BIGGEST_USV as i32 {
            ...
            if cur_cmd > ACTIVE_CHAR || cur_chr > BIGGEST_USV as i32 {
                cur_cmd = RELAX;
                cur_chr = TOO_BIG_USV;
            }
            if this_if == 0 {
        ...
        IF_CS_CODE => {
            ...
            flush_list(Some(n));
            b = Cmd::from(EQTB[cur_cs as usize].cmd) != UNDEFINED_CS;
            is_in_csname = e;
            current_block = 16915215315900843183;
        }
        IF_IN_CSNAME_CODE => {
...
pub(crate) unsafe fn get_preamble_token() {
    loop {
        ...
        if !(cur_cmd == ASSIGN_GLUE && cur_chr == GLUE_BASE as i32 + GluePar::tab_skip as i32) {
            break;
        }
...
pub(crate) unsafe fn init_align() {
    ...
    loop {
        ...
        loop {
            ...
            if cur_cmd <= CAR_RET && cur_cmd >= TAB_MARK && align_state as i64 == -1000000 {
                if p == HOLD_HEAD as i32 && cur_loop.is_texnull() && cur_cmd == TAB_MARK {
                    cur_loop = cur_align
                } else {
                    if file_line_error_style_p != 0 {
                        print_file_line();
            ...
            } else if cur_cmd != SPACER || p != HOLD_HEAD as i32 {
...
pub(crate) unsafe fn fin_col() -> bool {
    ...
    if MEM[(cur_align + 5) as usize].b32.s0 != SPAN_CODE {
        ...
        set_NODE_type(u as usize, UNSET_NODE);
...
pub(crate) unsafe fn fin_align() {
    ...
    loop {
        ...
        set_NODE_type(q as usize, UNSET_NODE);
        ...
        if !is_char_node(q) {
            if NODE_type(q as usize) == UNSET_NODE {
                ...
                if cur_list.mode as i32 == -1 {
                    ...
                    set_NODE_type(q as usize, VLIST_NODE);
                ...
                loop {
                    ...
                    if cur_list.mode == -1 {
                        ...
                        MEM[(r + 1) as usize].b32.s1 = w;
                        set_NODE_type(r as usize, HLIST_NODE);
                    } else {
                        ...
                        set_NODE_type(r as usize, VLIST_NODE);
                    }
                    MEM[(r + 4) as usize].b32.s1 = 0;
...
pub(crate) unsafe fn align_peek() {
    loop {
        ...
        if cur_cmd == NO_ALIGN {
            ...
            if cur_cmd == CAR_RET && cur_chr == CR_CR_CODE {
                continue;
            }
            init_row();
            init_col();
...
pub(crate) unsafe fn show_save_groups() {
    unsafe fn do_loop(mut p: usize, mut a: i8) -> (bool, usize, i8) {
        ...
        match cur_group {
            ...
            GroupCode::MATH_LEFT => {
                if MEM[NEST[p + 1].eTeX_aux.unwrap()].b16.s1 == LEFT_NOAD.u16() {
                    print_esc_cstr(b"left");
                } else {
                    print_esc_cstr(b"middle");
                }
    ...
    loop {
        ...
        match current_block {
            9007357115414505193 => {
                ...
                if NODE_type(p as usize).u16() < GLUE_NODE.u16()
                    || NODE_type(p as usize).u16() > KERN_NODE.u16()
                {
                    current_block = 10249009913728301645;
                } else {
                    current_block = 11492179201936201469;
...
pub(crate) unsafe fn box_end(mut box_context: i32) {
    ...
    if box_context < BOX_FLAG {
        ...
        if box_context > SHIP_OUT_FLAG {
            ...
            if cur_cmd == HSKIP && cur_list.mode.abs() != VMODE
                || cur_cmd == VSKIP && cur_list.mode.abs() == VMODE
            {
                append_glue();
...
pub(crate) unsafe fn begin_insert_or_adjust() {
    ...
    if cur_cmd == VADJUST && scan_keyword(b"pre") {
...
pub(crate) unsafe fn make_mark() {
    ...
    let p = get_node(SMALL_NODE_SIZE);
    MEM[p + 1].b32.s0 = c;
    set_NODE_type(p as usize, MARK_NODE);
    MEM[p].b16.s0 = 0;
...
pub(crate) unsafe fn delete_last() {
    if cur_list.mode == VMODE && cur_list.tail == cur_list.head {
        ...
        if cur_chr != GLUE_NODE.u16() as i32 || last_glue != MAX_HALFWORD {
            ...
            if cur_chr == KERN_NODE.u16() as i32 {
                help_line[0] = b"Try `I\\kern-\\lastkern\' instead."
            } else if cur_chr != GLUE_NODE.u16() as i32 {
...
pub(crate) unsafe fn do_register_command(mut a: small_number) {
    ...
    if q != REGISTER {
        ...
        if cur_cmd >= ASSIGN_INT && cur_cmd <= ASSIGN_MU_GLUE {
            l = cur_chr;
            p = cur_cmd as u8 - ASSIGN_INT as u8;
...
pub(crate) unsafe fn do_extension() {
    ...
    match cur_chr {
        ...
        4 => {
            get_x_token();
            if cur_cmd == EXTENSION && cur_chr <= CLOSE_NODE as i32 {
                p = cur_list.tail;
                do_extension();
...
pub(crate) unsafe fn handle_right_brace() {
    ...
    match cur_group {
        ...
        GroupCode::VCENTER => {
            ...
            cur_list.tail = *LLIST_link(cur_list.tail) as usize;
            MEM[cur_list.tail].b16.s1 = VCENTER_NOAD.u16();
...
pub(crate) unsafe fn do_assignments() {
    loop {
        ...
        if cur_cmd as u8 <= MAX_NON_PREFIXED_COMMAND as u8 {
            return;
        }
