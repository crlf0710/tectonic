...
pub(crate) unsafe fn new_param_glue(mut n: i16) -> usize {
    let p = get_node(MEDIUM_NODE_SIZE);
...
pub(crate) unsafe fn new_skip_param(mut n: i16) -> usize {
    ...
    while p > 0 {
        if is_char_node(p) {
            ...
            match TextNode::n(MEM[p as usize].b16.s1).unwrap() {
                ...
                TextNode::Ligature => short_display(MEM[(p + 1) as usize].b32.s1),
                TextNode::Disc => {
...
pub(crate) unsafe fn eq_define(p: usize, t: Cmd, e: i32) {
    if Cmd::from(EQTB[p].cmd) == t && EQTB[p].val == e {
        eq_destroy(EQTB[p]);
        return;
    }
...
pub(crate) unsafe fn geq_define(p: usize, t: Cmd, e: i32) {
    eq_destroy(EQTB[p]);
...
pub(crate) unsafe fn get_next() {
    ...
    'c_63502: loop {
        ...
        if cur_cmd <= Cmd::CarRet && cur_cmd >= Cmd::TabMark && align_state == 0 {
            ...
            cur_cmd = Cmd::from(MEM[(cur_align + 5) as usize].b32.s0 as u16);
            ...
            match cur_cmd {
                ...
                Cmd::CSName => {
                    ...
                    if Cmd::from(EQTB[cur_cs as usize].cmd) == Cmd::UndefinedCS {
                        eq_define(cur_cs as usize, Cmd::Relax, TOO_BIG_USV);
                    }
                ...
                Cmd::FiOrElse => {
                    ...
                    if cur_chr > if_limit as i32 {
                        ...
                        cur_if = MEM[p as usize].b16.s0 as i16;
    ...
    match this_if {
        ...
        IF_VOID_CODE | IF_HBOX_CODE | IF_VBOX_CODE => {
            ...
            if this_if == IF_VOID_CODE {
                ...
                b = NODE_type(p as usize) == TextNode::HList.into();
            } else {
                b = NODE_type(p as usize) == TextNode::VList.into();
            }
            current_block = 16915215315900843183;
        }
        IFX_CODE => {
            ...
            if cur_cmd != p {
                ...
                let mut p = MEM[cur_chr as usize].b32.s1;
        ...
        loop {
            ...
            if offs > 0 {
                if prevOffs != 0 {
                    ...
                    if use_skip {
                        MEM[cur_list.tail].b32.s1 = new_param_glue(15 as i16) as i32;
                        cur_list.tail = *LLIST_link(cur_list.tail) as usize;
                    }
                }
...
pub(crate) unsafe fn hpack(mut p: i32, mut w: scaled_t, mut m: i16) -> i32 {
    let mut current_block: u64;
    ...
    's_130: while !p.is_texnull() {
        ...
        match current_block {
            10435735846551762309 => {
                let mut k = if q != r + 5 && NODE_type(q as usize) == TextNode::Disc.into() {
...
pub(crate) unsafe fn vpackage(mut p: i32, mut h: scaled_t, mut m: i16, mut l: scaled_t) -> usize {
    let mut current_block: u64;
    last_badness = 0;
...
pub(crate) unsafe fn append_to_vlist(b: usize) {
    ...
    if cur_list.aux.b32.s1 > IGNORE_DEPTH {
        ...
        if d < *DIMENPAR(DimenPar::line_skip_limit) {
            p = new_param_glue(GluePar::line_skip as i16) as i32;
        } else {
            p = new_skip_param(GluePar::baseline_skip as i16) as i32;
...
pub(crate) unsafe fn init_align() {
    ...
    align_state = -1000000;
    loop {
        MEM[cur_align as usize].b32.s1 = new_param_glue(GluePar::tab_skip as i16) as i32;
        /*:808 */
        ...
        loop {
            ...
            if cur_cmd <= Cmd::CarRet && cur_cmd >= Cmd::TabMark && align_state as i64 == -1000000 {
                if p == HOLD_HEAD as i32 && cur_loop.is_texnull() && cur_cmd == Cmd::TabMark {
                    cur_loop = cur_align
                } else {
                    if file_line_error_style_p != 0 {
                        print_file_line();
...
pub(crate) unsafe fn fin_col() -> bool {
    ...
    if MEM[(cur_align + 5) as usize].b32.s0 != SPAN_CODE {
        ...
        if cur_list.mode == (true, ListMode::HMode) {
            ...
            u = hpack(MEM[cur_list.head].b32.s1, 0, ADDITIONAL as i16);
            ...
            u = vpackage(MEM[cur_list.head].b32.s1, 0, ADDITIONAL as i16, 0) as i32;
        ...
        set_NODE_type(u as usize, TextNode::Unset);
...
pub(crate) unsafe fn fin_row() {
    let mut p: i32 = 0;
    if cur_list.mode == (true, ListMode::HMode) {
        p = hpack(MEM[cur_list.head].b32.s1, 0, ADDITIONAL as i16);
        pop_nest();
...
pub(crate) unsafe fn fin_align() {
    ...
    if cur_list.mode == (true, ListMode::VMode) {
        ...
        *DIMENPAR(DimenPar::overfull_rule) = 0;
        p = hpack(
            ...
            SAVE_STACK[SAVE_PTR + 0].val as i16,
        );
    ...
    while !q.is_texnull() {
        if !is_char_node(q) {
            if NODE_type(q as usize) == TextNode::Unset.into() {
                /*836: */
                if cur_list.mode == (true, ListMode::VMode) {
                    set_NODE_type(q as usize, TextNode::HList);
                    MEM[(q + 1) as usize].b32.s1 = MEM[(p + 1) as usize].b32.s1;
                    if NEST[(NEST_PTR - 1) as usize].mode == (false, ListMode::MMode) {
                        MEM[q as usize].b16.s0 = LRMode::DList as u16;
                    }
                } else {
                    set_NODE_type(q as usize, TextNode::VList);
                ...
                loop {
                    ...
                    while n > 0 {
                        ...
                        if cur_list.mode == (true, ListMode::VMode) {
                            ...
                            set_NODE_type(u as usize, TextNode::VList);
                    ...
                    if cur_list.mode == (true, ListMode::VMode) {
                        ...
                        MEM[(r + 1) as usize].b32.s1 = w;
                        set_NODE_type(r as usize, TextNode::HList);
                    } else {
                        ...
                        set_NODE_type(r as usize, TextNode::VList);
                    }
                    MEM[(r + 4) as usize].b32.s1 = 0;
            ...
            } else if NODE_type(q as usize) == TextNode::Rule.into() {
                /*835: */
                if MEM[(q + 1) as usize].b32.s1 == NULL_FLAG {
                    MEM[(q + 1) as usize].b32.s1 = MEM[(p + 1) as usize].b32.s1
                }
    ...
    if cur_list.mode == (false, ListMode::MMode) {
        ...
        cur_list.tail = *LLIST_link(cur_list.tail) as usize;
        MEM[cur_list.tail].b32.s1 = new_param_glue(GluePar::above_display_skip as i16) as i32;
        cur_list.tail = *LLIST_link(cur_list.tail) as usize;
        MEM[cur_list.tail].b32.s1 = p;
        if !p.is_texnull() {
        ...
        cur_list.tail = *LLIST_link(cur_list.tail) as usize;
        MEM[cur_list.tail].b32.s1 = new_param_glue(GluePar::below_display_skip as i16) as i32;
        cur_list.tail = *LLIST_link(cur_list.tail) as usize;
...
pub(crate) unsafe fn vsplit(mut n: i32, mut h: scaled_t) -> i32 {
    ...
    if !q.is_texnull() {
        q = vpackage(q, 0, ADDITIONAL as i16, MAX_HALFWORD) as i32
    }
    if cur_val < 256 {
...
pub(crate) unsafe fn app_space() {
    ...
    if cur_list.aux.b32.s0 >= 2000 && *GLUEPAR(GluePar::xspace_skip) != 0i32 {
        q = new_param_glue(GluePar::xspace_skip as i16)
    } else {
...
pub(crate) unsafe fn normal_paragraph() {
    ...
    if !EQTB[INTER_LINE_PENALTIES_LOC].val.is_texnull() {
        eq_define(INTER_LINE_PENALTIES_LOC, Cmd::ShapeRef, TEX_NULL);
    };
}
    ...
    *INTPAR(IntPar::xetex_upwards) = u;
    if cur_list.mode == (true, ListMode::HMode) {
        cur_box = hpack(
            ...
            SAVE_STACK[SAVE_PTR + 1].val as i16,
        )
    } else {
        cur_box = vpackage(
            MEM[cur_list.head].b32.s1,
...
pub(crate) unsafe fn new_graf(mut indented: bool) {
    ...
    if cur_list.mode == (false, ListMode::VMode) || cur_list.head != cur_list.tail {
        MEM[cur_list.tail].b32.s1 = new_param_glue(GluePar::par_skip as i16) as i32;
        cur_list.tail = *LLIST_link(cur_list.tail) as usize;
    }
    push_nest();
...
pub(crate) unsafe fn unpackage() {
    ...
    if cur_chr > COPY_CODE {
        ...
        if cur_val < 256 {
            ...
            find_sa_element(4 as i16, cur_val, false);
            if cur_ptr.is_texnull() {
                p = TEX_NULL;
            } else {
                p = MEM[(cur_ptr + 1) as usize].b32.s1;
        ...
        if cur_list.mode.1 == ListMode::MMode
            || cur_list.mode.1 == ListMode::VMode && NODE_type(p as usize) != TextNode::VList.into()
            || cur_list.mode.1 == ListMode::HMode && NODE_type(p as usize) != TextNode::HList.into()
        {
            if file_line_error_style_p != 0 {
                print_file_line();
            } else {
    ...
    while !MEM[cur_list.tail].b32.s1.is_texnull() {
        ...
        if !is_char_node(r) && NODE_type(r as usize) == TextNode::MarginKern.into() {
...
pub(crate) unsafe fn make_accent() {
    ...
    if let Some(mut p) = new_character(f, cur_val as UTF16_code) {
        ...
        if let Some(q) = q {
            ...
            if h != x {
                p = hpack(p as i32, 0, ADDITIONAL as i16) as usize;
                MEM[p + 4].b32.s1 = x - h;
            }
...
pub(crate) unsafe fn just_reverse(mut p: i32) {
    ...
    while !q.is_texnull() {
        if is_char_node(q) {
            ...
            p = q;
            q = MEM[p as usize].b32.s1;
            if NODE_type(p as usize) == TextNode::Math.into() {
                /*1527: */
                if MEM[p as usize].b16.s0 as i32 & 1 != 0 {
                    if MEM[LR_ptr as usize].b32.s0
...
pub(crate) unsafe fn do_register_command(mut a: i16) {
    ...
    if p < GLUE_VAL {
        ...
        if e {
            ...
            geq_define(l as usize, Cmd::GlueRef, cur_val);
        } else {
            eq_define(l as usize, Cmd::GlueRef, cur_val);
        }
    };
}
...
pub(crate) unsafe fn new_font(mut a: i16) {
    ...
    if a >= 4 {
        geq_define(u as usize, Cmd::SetFont, FONT_BASE as i32);
    } else {
        eq_define(u as usize, Cmd::SetFont, FONT_BASE as i32);
    }
    scan_optional_equals();
    scan_file_name();
    name_in_progress = true;
    ...
    if a >= 4 {
        geq_define(u as usize, Cmd::SetFont, f as i32);
    } else {
        eq_define(u as usize, Cmd::SetFont, f as i32);
    }
        ...
        6 => {
            ...
            if cond_ptr.is_texnull() {
                ...
                loop {
                    ...
                    n -= 1;
                    t = MEM[p as usize].b16.s0 as i16;
...
pub(crate) unsafe fn handle_right_brace() {
    ...
    match cur_group {
        ...
        GroupCode::MATH => {
            ...
            if !p.is_texnull() {
                if MEM[p as usize].b32.s1.is_texnull() {
                    if MEM[p as usize].b16.s1 == MathNode::Ord as u16 {
                    ...
                    } else if MEM[p as usize].b16.s1 == MathNode::Accent as u16 {
        ...
        match current_block {
            ...
            /*append_normal_space */
            {
                ...
                if *GLUEPAR(GluePar::space_skip) == 0 {
                    ...
                    temp_ptr = new_param_glue(GluePar::space_skip as i16) as i32
                }
...
pub(crate) unsafe fn do_marks(mut a: i16, mut l: i16, mut q: i32) -> bool {
    if l < 4 {
        for i in 0..=15 {
            ...
            if !cur_ptr.is_texnull() {
                if do_marks(a, (l as i32 + 1i32) as i16, cur_ptr) {
                    if i as i32 & 1i32 != 0 {
